...
interface SessionAccordion {
  session: RouterOutputs["elections"]["sessions"][0];
}
...
export function SessionAccordion({ session }: SessionAccordion) {
        ...
        <AccordionTrigger>
          <div className="flex items-center justify-between px-4 py-2">
            <div className="flex items-center space-x-2">
              <div className="mr-1 text-sm font-medium">{session.name}</div>
            </div>
          </div>
        </AccordionTrigger>
        <AccordionContent>
                                  ...
                                  <Form {...candidateForm}>
                                    <form
                                      onSubmit={candidateForm.handleSubmit(
                                        (data) =>
                                          onCandidateCreated(data, position.id),
                                      )}
                                    >
                                      <FormField
                          ...
                          <TableBody>
                            {session.statuteChanges.map((statuteChange) => (
                              <TableRow key={statuteChange.id}>
                                <TableCell>{statuteChange.name}</TableCell>
                                <TableCell>
                                  {statuteChange.withAbstain ? "Yes" : "No"}
                                </TableCell>
                                <TableCell>
                                  <PopoverActions
